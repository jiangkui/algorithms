...
### 3.1.5 有序数组中的二分查找
有序符号表的实现，它使用的数据结构是两个数组，一个存键，一个存值。
...
| 使用数据结构     	|     优点  							|  缺点   								|
| :-------- 					| :--------							| :------ 									|
| 链表（顺序查找）|   适用于小型问题			|  对大型表很慢					|
| 有序数组（二分查找）| 最优的查找效率和空间需求能够进行有序性相关的操作| 插入操作很慢|
| 二叉查找树			| 实现简单，能够进行有序性相关的操作|没有性能上界的保证链接需要额外的空间|
|平衡二叉查找树		| 最优的查找和插入效率，能够进行有序性相关的操作|链接需要额外的空间|
|散列表					|能够快速的查找和插入常见类型的数据 | 需要计算每种类型的数据的散列  		无法进行有序性相关的操作 			链接和空节点需要额外的空间|

## 3.2 二叉查找树（BST）
一颗二叉查找树（BST）是一颗二叉树，其中每个节点都含有一个 Comparable 的键（以及相关联的值）且每个节点的键都大于其左子树中的任意节点的键而小于右子树的任意节点的键。
...
二叉树的 put 
比当前节点的值大就往 右子树插入，比当前的值小，就往左子树插入。
但这样产生的树不是平衡的。
...
## 3.3 平衡查找树
在动态插入中保持树的完美平衡的代价太高了，我们稍稍放松完美平衡的要求并将学习一种能够保证符号表 API 中所有操作 均能够在对数时间内完成的数据结构（范围查找除外）。

`ps：刚才休息了以下，考虑了接下来两个多月的学习计划，先把查找算法处理完，之后学 linux，同时学一个脚本语言 shell 或 python，预计时间一个月，之后 看一下 servlet，1~2周，加深了解下 spring 的东西，1周左右，在接下来 看并发，jvm 的一些东西，最后 mysql 的查询优化，索引等。这样应该就差不多到明年的三月份了。 面个 P5 应该没问题`
### 3.3.1 2-3 查找树
定义：一颗 2-3 查找树有以下节点组成：
- 2-节点，含有一个键和两条链接，左链接指的键 都小于该节点，右链接的键都大于该节点。
- 3-节点，含有两个键和三条链接，左链接的键都小于该节点，中链接的键都位于该节点的两个键之间，右链接的键都大于该节点。

一颗 完美平衡的 2-3 查找树中所有空连接到根节点的距离都应该是相同的。

我们使用 2-3 树的主要原因在于，它能够在插入后继续保持平衡。

`我的理解：`插入的大概逻辑为
如果插入的位置是个 2 -节点（一个key），则把他转成 3-节点即可（2个key）。
如果插入的位置是个 3-节点（2个key），则把它变成 4-节点（3个key），之后吧中间的key 挤到父节点上。如果 父节点只有一个key，则加一个key 即可，如果父节点有 2个key，则重复这个操作（再挤到它的父节点，最终递归到 root 上，然后修改root 就ok）

逻辑是这样，实现起来可能会比较麻烦。
平衡一个棵树的初衷是为了消除最坏情况，但我们希望这种保障所需的代码能够越少越好。
`end:2015-12-06 18:30`
### 3.3.2 红黑二叉查找树

### 3.3.3 实现
`start:2015-12-06 23:08`
`实现代码跳过`

## 3.4 散列表
简单的介绍了散列表：
- 散列函数：将被查找的键转化为数组的索引
- 处理碰撞冲突的过程：链表和线形探测法（在一个数组中循环）

## 3.5 应用
高效的查找算法确保了无数重要的应用程序称为可能。

第六章中，我们会学习一种适合于数据库或者文件系统的符号表，他能够保存的数量超过你的想象。


### 3.5.1 我们该使用符号表的那种实现
散列表的优点在于代码更简单，且查找时间最优。但设置高效的散列函数。
...
- 散列表（拉链法）：java.util.HashMap ，它也会自动调整数组的大小来保持使用率大约不超过 75%
...
## 5.3 子字符串查找
给定一段长度为 N 的文本和一个长度为 M 的模式字符串，在文本中找到一个和该模式相符的子字符串。 

字符串查找是一个很有趣而且很经典的问题：人们发明了几个截然不同（且令人惊讶的）算法，他们不禁产生了一系列能够实际应用的查找方法，而且也展示了许多重要的算法设计技巧。
...
### 5.5.6 霍夫曼压缩
`思想：`通过使用较少的比特表示出现频发的字符而用较多的比特表示偶尔出现的字符来节省空间。

`end:2015-12-06 23:57`
# 第 6 章 背景